...
export class AlchemyGasClient {
  ...
  async getCurrentGasPrice(chain: string): Promise<AlchemyGasPrice> {
    try {
      ...
      const baseFee = Number.parseInt(data.result.baseFeePerGas[0], 16) / 1e9
      const priorityFee = data.result.reward[0] ? Number.parseInt(data.result.reward[0][1], 16) / 1e9 : 0
  ...
  async getGasHistory(chain: string, blockCount = 100): Promise<AlchemyGasPrice[]> {
    try {
      ...
      const blockTime = chain === "ethereum" ? 12000 : chain === "polygon" ? 2000 : 250
